...
class PredictiveEngineTest {
  ...
  private CycleContext context(Money targetSavings, Money flexibleBudgetCap) {
    return new CycleContext(
        new DateInterval(CYCLE_START, CYCLE_END), TODAY, targetSavings, flexibleBudgetCap);
  }

  private TransactionSnapshot confirmedFlexible(Money amount, LocalDate date) {
    return snapshot(TransactionType.FLEXIBLE_EXPENSE, TransactionStatus.CONFIRMED, amount, date);
  }
  ...
  void noFlexibleSpendingSoFarGivesFullDailyAllowance() {
    ...
    assertThat(result.remainingFlexible().getValue())
  ...
  void overspendYesterdayRecalibratesDown() {
    ...
    var result =
        engine.calculate(
            ...
            new CurrentState(
                money(1000), List.of(confirmedFlexible(money(800), LocalDate.of(2026, 9, 12)))));
  ...
  void underspendYesterdayRecalibratesUp() {
    ...
    var result =
        engine.calculate(
            context(money(0), money(900)),
            new CurrentState(
                money(500), List.of(confirmedFlexible(money(100), LocalDate.of(2026, 9, 12)))));
  ...
  void negativeProjectedBalanceButPositiveFlexCapacityIsRestricted() {
    ...
    var result =
        engine.calculate(
            ...
            new CurrentState(
                ...
                List.of(
                    projected(
                        TransactionType.FIXED_EXPENSE, money(200), LocalDate.of(2026, 9, 15)))));
    ...
    assertThat(result.projectedBalance().getValue())
        .isEqualByComparingTo(new BigDecimal("-100.00"));
  }
  ...
  void lastDayOfCycleHasSingleDayRemaining() {
    var lastDayContext =
        new CycleContext(new DateInterval(CYCLE_START, CYCLE_END), CYCLE_END, money(0), money(300));

    var result = engine.calculate(lastDayContext, new CurrentState(money(300), List.of()));
  ...
  void flexibleSpendingOutsideCycleDoesNotConsumeBudget() {
    var result =
        engine.calculate(
            context(money(0), money(900)),
            new CurrentState(
                money(1000), List.of(confirmedFlexible(money(500), LocalDate.of(2026, 8, 15)))));
  ...
  void targetSavingsIsReserveBeforeFlexibleBudget() {
    ...
    var result =
        engine.calculate(context(money(400), money(900)), new CurrentState(money(1000), List.of()));
  ...
  private static TransactionSnapshot snapshot(
      TransactionType type, TransactionStatus status, Money amount, LocalDate dueDate) {
