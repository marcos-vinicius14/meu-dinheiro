...
class WhatIfSimulatorTest {
  ...
  private CycleContext baseContext() {
    return new CycleContext(
        new DateInterval(CYCLE_START, CYCLE_END), TODAY, money(100), money(900));
  }
  ...
  void singlePurchaseInCurrentCycleDegradesS2S() {
    ...
    var result =
        simulator.simulate(baseContext(), baseState(), new SimulatedPurchase(money(300), 1, TODAY));

    assertThat(result).hasSize(1);

    var first = result.get(0);
    assertThat(first.cycleInterval().startDate()).isEqualTo(CYCLE_START);
    assertThat(first.engineResult().s2sToday().getValue())
  ...
  void installmentPurchaseProjectsAcrossSixCycles() {
    ...
    // Setembro: NetLiquidity = 1000 + 500 - 300 - (200 + 100) - 100 = 800 → S2S = floor(800/18) =
    // 44.44
    // Out/Nov/Dez/Jan/Fev: sem a renda/despesas de setembro (estado base só cobre setembro);
    // cada ciclo: liquidez 1000 - 100 (parcela) - 100 (reserva) = 800, cap 900 → S2S =
    // floor(800/30)
    var result =
        simulator.simulate(baseContext(), baseState(), new SimulatedPurchase(money(600), 6, TODAY));
  ...
  void twelveMonthSimulationReportsBottleneckWhenLiquidityRunsOut() {
    ...
    // Compra de 24000 em 12x = 2000/mês → NetLiquidity = 1000 - 2000 - 100 < 0 → DEFICIT_RISK em
    // todos
    var result =
        simulator.simulate(
            baseContext(), baseState(), new SimulatedPurchase(money(24000), 12, TODAY));
  ...
  void bottleneckIdentifiesCyclesThatDegradeHealth() {
    ...
    var result =
        simulator.simulate(
            baseContext(), baseState(), new SimulatedPurchase(money(12000), 12, TODAY));
  ...
  void noImpactWhenPurchaseIsZero() {
    var result =
        simulator.simulate(baseContext(), baseState(), new SimulatedPurchase(money(0), 1, TODAY));
