...
/**
 * Rate limit por IP no endpoint. Cada teste usa um IP proprio via header X-Forwarded-For (cenario
 * real: cliente atras de proxy), isolando os contadores entre testes sem contexto Spring separado.
 */
...
class LoginEndpointRateLimitIT extends IntegrationTestSupport {
  ...
  private String loginFrom(String ip, String email, String password, int expectedStatus) {
    ...
    var response = restTemplate.exchange("/auth/login", HttpMethod.POST, entity, String.class);
