...
class BankAccountModelTest {
  ...
  void createBankAccountStoresValues() {
    var user = createUser();
    var account =
        BankAccountModel.create("Carteira", BankAccountType.CASH, new Money(BigDecimal.TEN), user);
  ...
  void belongsToReturnsTrueForSameUser() {
    ...
    var account =
        BankAccountModel.create(
            "Conta", BankAccountType.CHECKING, new Money(BigDecimal.ZERO), user);
  ...
  void belongsToReturnsFalseForDifferentUser() {
    ...
    var account =
        BankAccountModel.create(
            "Conta", BankAccountType.CHECKING, new Money(BigDecimal.ZERO), owner);
  ...
  void updateNameChangesName() {
    var account =
        BankAccountModel.create(
            "Antigo", BankAccountType.CHECKING, new Money(BigDecimal.ZERO), createUser());
  ...
  void updateInitialBalanceChangesBalance() {
    var account =
        BankAccountModel.create(
            "Conta", BankAccountType.CHECKING, new Money(BigDecimal.ZERO), createUser());

    account.updateInitialBalance(new Money(new BigDecimal("150.75")));

    assertThat(account.getInitialBalance().getValue())
        .isEqualByComparingTo(new BigDecimal("150.75"));
  }

  private UserModel createUser() {
    var email = Email.create("user-" + UUID.randomUUID() + "@example.com");
    var user = UserModel.create("Test User", email.value(), "password123");
