...
/**
 * Suporte compartilhado para ITs: autenticação real, helpers de criação de recursos via API e
 * extração de campos JSON sem parsing por string.
 *
 * <p>HTTP: {@link #restTemplate} fala com o servidor real (RANDOM_PORT, Tomcat).
 */
...
public abstract class IntegrationTestSupport extends AbstractIntegrationTest {
  ...
  /**
   * Limpeza de dados de domínio após cada classe de teste: mantém o container singleton previsível
   * entre suítes sem recriar o contexto Spring. Preserva tb_users? Não — remove usuários e tudo que
   * casca em cascata. Chaves/segurança não são tocadas.
   */
  @AfterAll
  ...
  /**
   * Login via HTTP real (porta aleatória): captura os Set-Cookie emitidos pelo Tomcat. Use este
   * helper quando o teste valida o comportamento do endpoint em si (cookies, headers, rotação).
   */
  ...
  private static String cookieFromHeader(java.util.List<String> setCookies, String name) {
  ...
  private static java.util.stream.Stream<String> extractCookieValue(
      String name, String setCookieHeader) {
    if (!setCookieHeader.startsWith(name + "=")) {
      return java.util.stream.Stream.empty();
    }

    var value = setCookieHeader.substring((name + "=").length());
  ...
  protected UUID createCategory(Session session, String description, boolean flexible) {
    var response =
  ...
  protected UUID createAccount(Session session, String name, String type, String initialBalance) {
    var response =
        authenticated(session)
            .post(
                "/bankaccounts",
  ...
  protected UUID createTransaction(
      Session session, String description, String amount, String type, UUID categoryId) {
  ...
  protected UUID findUserIdByEmail(String email) {
    return jdbcTemplate.queryForObject(
        "SELECT id FROM tb_users WHERE email = ?", UUID.class, email);
  }
  ...
  protected AuthenticatedRequest authenticated(Session session) {
    return new AuthenticatedRequest(ACCESS_TOKEN_COOKIE + "=" + session.accessToken());
  }
  ...
  public class AuthenticatedRequest {
    ...
    private HttpResult exchange(
        org.springframework.http.HttpMethod method, String uri, String body) {
