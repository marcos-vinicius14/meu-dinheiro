...
public class SimulatePurchaseUseCaseImpl implements SimulatePurchaseUseCase {
  ...
  public OperationResult<SimulationOutput> execute(UUID userId, SimulationInput input) {
    ...
    var cycleInterval =
        new DateInterval(
            YearMonth.from(input.date()).atDay(1), YearMonth.from(input.date()).atEndOfMonth());
    ...
    try {
      ...
      var cycles = simulations.stream().map(SimulatePurchaseUseCaseImpl::toDto).toList();
