...
public class TransactionBundleModel {
    ...
    public static TransactionBundleModel create(
        ...
        return new TransactionBundleModel(description, totalAmount, totalInstallments, firstDueDate, user);
    }

    /**
     * Gera as N parcelas filhas (COMMITTED) com alocação HALF_EVEN: a primeira
     * parcela absorve o resto de arredondamento, garantindo soma exata do total.
     * Vencimentos mensais a partir de firstDueDate; dia clampado ao fim do mês.
     * bundleId é o id do bundle já persistido; user/category são propagados.
     */
    public List<TransactionModel> generateInstallments(
        ...
        for (int i = 0; i < totalInstallments; i++) {
            installments.add(TransactionModel.createInstallment(
                    description,
                    allocations.get(i),
