...
/**
 * Serviço de domínio puro: sem Spring, sem I/O, determinístico.
 * Implementa as regras algorítmicas de S2S (safe-to-spend) do ciclo.
 */
public class PredictiveEngine {
    ...
    public EngineResult calculate(CycleContext context, CurrentState state) {
        ...
        var fixedExpenses = sumForward(state.transactions(), today, interval, TransactionType.FIXED_EXPENSE);
        var committedExpenses = sumForwardCommittedInstallments(state.transactions(), today, interval);

        var remainingBudget = Money.max(
                context.flexibleBudgetCap().subtract(flexSpent),
                Money.zero()
        );

        var netLiquidityBeforeFlex = state.liquidBalance()
                .add(projectedIncome)
        ...
        if (remainingFlexible.isNegative() || remainingFlexible.isZero()) {
            ...
            healthStatus = projectedBalance.isNegative()
    ...
    private Money sumUntilYesterday(
        ...
        return Money.sum(transactions.stream()
