...
/**
 * Serviço de domínio puro: simula o impacto de uma compra ao longo de N ciclos mensais sem
 * persistência. Reutiliza a PredictiveEngine para cada ciclo projetado.
 */
public class WhatIfSimulator {
  ...
  public List<CycleSimulation> simulate(
      CycleContext baseContext, CurrentState baseState, SimulatedPurchase purchase) {
    ...
    for (int i = 0; i < purchase.installments(); i++) {
      ...
      var cycleInterval = new DateInterval(cycleMonth.atDay(1), cycleMonth.atEndOfMonth());
      var cycleCurrentDate =
          cycleMonth.equals(firstCycleMonth)
      ...
      var installmentDueDate =
          installmentDueDateInCycle(purchase.firstDueDate(), purchaseStartMonth, cycleMonth);
      ...
      var reductionPercent =
          baselineS2S.isZero() ? BigDecimal.ZERO : reduction.percentOver(baselineS2S);

      var bottleneck =
          result.healthStatus() == HealthStatus.DEFICIT_RISK
  ...
  /**
   * Data da parcela deste ciclo: mesmo day-of-month da primeira parcela, ajustado para o fim do mês
   * quando o dia não existe.
   */
  private @Nullable LocalDate installmentDueDateInCycle(
      LocalDate firstDueDate, YearMonth purchaseStartMonth, YearMonth cycleMonth) {
    var monthsBetween =
        (int) java.time.temporal.ChronoUnit.MONTHS.between(purchaseStartMonth, cycleMonth);
    if (monthsBetween < 0) {
      return null;
    }
  ...
  /**
   * Estado dos ciclos futuros: sem liquidez herdada dos eventos do ciclo atual. Usa a liquidez base
   * como caixa inicial e apenas as transações recorrentes/parcelas.
   */
