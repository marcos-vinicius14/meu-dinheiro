...
public class BankAccountController {
  ...
  @PostMapping
  ResponseEntity<Object> create(@Valid @RequestBody CreateBankAccountRequest request) {
    var userId = currentIdentity.findCurrentAuthenticadedUser();
    var input =
        new CreateBankAccountInput(request.name(), request.type(), request.initialBalance());
    ...
    return ResponseEntity.status(HttpStatus.CREATED).body(BankAccountResponse.from(result.value()));
  }
  ...
  ResponseEntity<List<BankAccountResponse>> list() {
    ...
    var responses = result.value().stream().map(BankAccountResponse::from).toList();
  ...
  ResponseEntity<Object> update(
    ...
    var userId = currentIdentity.findCurrentAuthenticadedUser();
    var input = new UpdateBankAccountInput(request.name(), request.initialBalance());
  ...
  ResponseEntity<Object> delete(@PathVariable UUID id) {
    ...
    if (result.isFailure()) {
      ...
      if (BankAccountMessages.ACCOUNT_HAS_TRANSACTIONS.equals(error)) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(new ErrorResponse(result.errors()));
      }
