...
public class CategoryController {
  ...
  @PostMapping
  ResponseEntity<Object> create(@Valid @RequestBody CreateCategoryRequest request) {
    var userId = currentIdentity.findCurrentAuthenticadedUser();
    var input =
        new CreateCategoryInput(
            request.description(), request.icon(), request.isFlexibleOrDefault());
    ...
    return ResponseEntity.status(HttpStatus.CREATED).body(CategoryResponse.from(result.value()));
  }
  ...
  ResponseEntity<List<CategoryResponse>> list() {
    ...
    var responses = result.value().stream().map(CategoryResponse::from).toList();
  ...
  ResponseEntity<Object> update(
    ...
    var userId = currentIdentity.findCurrentAuthenticadedUser();
    var input = new UpdateCategoryInput(request.description(), request.icon());
