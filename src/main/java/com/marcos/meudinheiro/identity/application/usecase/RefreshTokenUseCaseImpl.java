...
public class RefreshTokenUseCaseImpl implements RefreshTokenUseCase {
  ...
  public OperationResult<AuthenticationOutput> execute(String rawRefreshToken) {
    ...
    var refreshToken = notification.collect(resolveRefreshToken(rawRefreshToken, now));
    ...
    var identity = notification.collect(resolveIdentity(refreshToken.getUserId()));

    if (notification.hasErrors()) {
      return OperationResult.failure(notification.errors());
    }

    var accessToken = jwtTokenService.generate(new JwtSubject(identity.id(), identity.email()));

    var generatedRefreshToken = refreshTokenGenerator.generate();

    var newRefreshToken =
        RefreshTokenModel.create(
            identity.id(), generatedRefreshToken.hash(), now.plus(REFRESH_TOKEN_TTL), now);
  ...
  private ValidationResult<RefreshTokenModel> resolveRefreshToken(
      String rawRefreshToken, Instant now) {
    ...
    var refreshToken =
        refreshTokenRepository
            .findByTokenHash(RefreshTokenHash.sha256(rawRefreshToken))
            .orElse(null);
  ...
  private ValidationResult<RefreshTokenModel> validateRefreshToken(
      RefreshTokenModel refreshToken, Instant now) {
