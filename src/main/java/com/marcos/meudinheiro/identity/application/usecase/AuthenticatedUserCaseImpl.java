...
public class AuthenticatedUserCaseImpl implements AuthenticateUserUseCase {
  ...
  public OperationResult<AuthenticationOutput> execute(AuthenticationInput authenticationInput) {
    var email = authenticationInput.email().trim().toLowerCase(Locale.ROOT);
      ...
      var authentication =
          authenticationManager.authenticate(
              UsernamePasswordAuthenticationToken.unauthenticated(
                  email, authenticationInput.password()));

      loginAttemptLimiter.reset(email);

      return OperationResult.success(
          issueSession(Objects.requireNonNull((AuthenticadedUser) authentication.getPrincipal())));
  ...
  private AuthenticationOutput issueSession(AuthenticadedUser principal) {
    var accessToken = jwtTokenService.generate(new JwtSubject(principal.id(), principal.email()));
    ...
    var refreshToken =
        RefreshTokenModel.create(principal.id(), generatedRefreshToken.hash(), expiresAt, now);
